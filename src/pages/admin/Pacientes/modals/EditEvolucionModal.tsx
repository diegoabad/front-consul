...
export function EditEvolucionModal({
  ...
  const { data: profesionales = [] } = useQuery({
    queryKey: ['profesionales', 'for-evoluciones-edit'],
  ...
  useEffect(() => {
    if (open && isProfesional && profesionalLogueado && evolucion) {
      setFormData(prev => ({
        ...prev,
        profesional_id: profesionalLogueado.id,
      }));
    }
  }, [open, isProfesional, profesionalLogueado, evolucion]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await onSubmit(evolucion.id, dataToSubmit);
      onOpenChange(false);
    } catch {
  ...
  const isFormValid = formData.fecha_consulta;
          ...
          <div className="flex-1 min-h-0 relative">
            ...
            <div className={`absolute inset-0 px-8 py-6 flex flex-col ${activeTab === 'evolucion' ? 'block' : 'hidden'}`}>
              <Textarea
                ...
                className="flex-1 min-h-0 border-[1.5px] border-[#D1D5DB] rounded-[10px] text-[16px] font-['Inter'] placeholder:text-[#9CA3AF] focus:border-[#7C3AED] focus:ring-2 focus:ring-[#7C3AED]/20 resize-none transition-all duration-200 leading-relaxed"
              />
