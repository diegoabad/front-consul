  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent hideCloseButton className="max-w-[90vw] w-[90vw] max-h-[90vh] h-[90vh] max-lg:max-h-[75vh] max-lg:h-[75vh] max-lg:max-w-[95vw] max-lg:w-[95vw] rounded-[20px] p-0 border border-[#E5E7EB] shadow-2xl flex flex-col overflow-hidden max-lg:rounded-[16px]">
        ...
        {/* Desktop: datos a un costado y visor al otro. Móvil: datos arriba, visor abajo */}
        <div className="flex-1 min-h-0 flex flex-col lg:flex-row overflow-hidden">
          {/* Datos: fecha, subido por, comentario */}
          <div className="flex-shrink-0 lg:w-[280px] lg:min-w-[240px] lg:border-r lg:border-b-0 border-b border-[#E5E7EB] px-6 py-4 max-lg:px-4 max-lg:py-3 space-y-3 bg-[#F9FAFB] lg:overflow-auto">
          ...
          <div className="flex-1 min-h-0 min-w-0 p-6 max-lg:p-3 max-lg:min-h-[120px] flex items-center justify-center bg-[#F9FAFB] overflow-auto">
            <div className="w-full h-full min-h-0 flex items-center justify-center">
              {isLoading ? (
                <div className="flex flex-col items-center gap-4">
                  <Loader2 className="h-12 w-12 animate-spin text-[#2563eb]" />
                  <p className="text-[#6B7280] font-['Inter']">
                    {isPdf(archivo.tipo_archivo) ? 'Cargando PDF...' : 'Cargando imagen...'}
                  </p>
                </div>
              ) : hasError ? (
                <div className="flex flex-col items-center gap-4 text-center">
                  <div className="h-20 w-20 rounded-full bg-[#FEE2E2] flex items-center justify-center">
                    {isPdf(archivo.tipo_archivo) ? (
                      <FileText className="h-10 w-10 text-[#EF4444]" />
                    ) : (
                      <Image className="h-10 w-10 text-[#EF4444]" />
                    )}
                  </div>
                  <p className="text-[#6B7280] font-['Inter']">
                    Error al cargar {isPdf(archivo.tipo_archivo) ? 'el PDF' : 'la imagen'}
                  </p>
                </div>
              ) : isPdf(archivo.tipo_archivo) ? (
                <iframe
                  ...
                  className="w-full h-full min-h-[300px] lg:min-h-0 rounded-[12px] border border-[#E5E7EB] bg-white shadow-lg"
                />
              ) : (
                <img
                  src={fileSrc}
