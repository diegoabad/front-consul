...
import { Building2, Plus, Pencil, Lock, Unlock, Trash2, Loader2, Search, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function AdminObrasSociales() {
  ...
  const { data: paginatedResponse, isLoading } = useQuery({
    queryKey: ['obras-sociales', 'paginated', page, limit, effectiveSearch],
    queryFn: () =>
      obrasSocialesService.getAllPaginated({
        page,
        limit,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-base text-[#6B7280] mt-2 font-['Inter']">
            {isLoading ? 'Cargando...' : totalPages > 0 ? `Mostrando ${(page - 1) * PAGE_SIZE + 1}-${Math.min(page * PAGE_SIZE, total)} de ${total} obras sociales` : `${total} ${total === 1 ? 'obra social registrada' : 'obras sociales registradas'}`}
          </p>
        </div>
        <Button
              ...
              {isLoading ? (
                <TableRow>
                  <TableCell colSpan={3} className="py-16 text-center">
                    ...
                    <p className="text-[#6B7280] font-['Inter'] text-sm m-0">Cargando obras sociales...</p>
                  </TableCell>
                </TableRow>
              ) : (
              items.map((item) => (
